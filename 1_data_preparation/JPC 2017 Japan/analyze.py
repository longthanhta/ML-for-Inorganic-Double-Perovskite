...
df_in = pd.read_excel('data.xlsx', sheet_name='perovskite')
...
X_list=[]
Eg_list=[]
for i in range(0,len(df_in['System'])):
    formula=df_in['System'][i]
...
#    print (i,formula[-4:])
    ...
    Eg_list.append(df_in['DirectGap_PBE'][i])
        

print (X_list)
'''
    B2=(df1['System'][i][8:9])
    if not hasNumbers(B2):
        B2_list.append(B2)
    B2_=(df1['System'][i][8:10])
    if not hasNumbers(B2_):
        B2_list.append(B2_)
'''       
'''       
    X=(df1['System'][i][8:9])
    if not hasNumbers(X):
        X_list.append(X)
    X_=(df1['System'][i][8:10])
    if not hasNumbers(X_):
        X_list.append(X_)
'''
#print (X_list)
...
df_out = pd.DataFrame.from_dict({'System':formula_list,'A':A_list,'B1':B1_list,'B2':B1_list,'B2':B2_list,'X':X_list,'Band_gap':Eg_list})

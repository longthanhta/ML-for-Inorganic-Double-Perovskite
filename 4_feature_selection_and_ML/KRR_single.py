...
df1 = pd.read_csv('single_perovskite_data_and_descriptors_final.csv')
...
for x in range(total_run_no):
    ...
    param_grid={"alpha":np.logspace(-5,5,15),"gamma":np.logspace(-5,5,15)}
    Y_average=np.mean(Y_train)
    kernel=Y_average*RBF(length_scale=1)

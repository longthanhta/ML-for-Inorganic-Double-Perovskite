...
df_features = pd.read_csv('result_A2_B1Q1_B2Q3_X6.csv')
...
#        print (format(row['A']),A_list[i],format(row['B1']),B1_list[i],format(row['B2']),B2_list[i],format(row['X']),X_list[i])
        if format(row['A'])==A_list[i] and ((format(row['B1'])==B1_list[i] and format(row['B2'])==B2_list[i]) or (format(row['B1'])==B2_list[i] and format(row['B2'])==B1_list[i])) and format(row['X'])==X_list[i] :
...
with open('data_and_descriptors_final_without_ABX.csv', 'w') as myfile:
